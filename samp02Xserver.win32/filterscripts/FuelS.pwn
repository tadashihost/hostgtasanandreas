    ...
    if	(IsPlayerAdmin(playerid))
        {
       	if	((strcmp(cmd,"/SetPlayerGas", true)==0)||(strcmp(cmd, "/SPG", true)==0))
			...
			if  (!IsPlayerInAnyVehicle(pid))
			    {
			    SendClientMessage(playerid, COLOR_WHITE, "Ese jugador esta a pie...");
			    return 1;
			    }
			new vid = GetPlayerVehicleID(playerid);
			if	((combustible[vid] + cant) > FULLTANK)
			...
			new string[255];
			new string2[255];
			format(string, sizeof(string), "Un ADMIN le puesto %d litros a tu vehiculo.", cant);
			format(string2, sizeof(string2), "Le has puesto %d litros al vehiculo.", cant);
