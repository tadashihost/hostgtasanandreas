...
#define YRACE_FILE "TurfWar/Races/yrace.rr"
#define RACE_FILE "TurfWar/Races/%s.yr"
...
new MajorityDelay = 120; // delay to wait for other racers once half are ready (can be changed ingame via Admin menu)
new RRotation = 0;      //  -1 = disabled, 0+ = enabled)
...
new PrizeMode=2;        //Mode for winnings: 0 - Fixed, 1 - Dynamic, 2 - Entry fee, 3 - EF+F, 4 EF+D [Admin menu ingame]
...
public OnFilterScriptInit()
{
	...
	Participants=0;
	for(new i;i<MAX_BUILDERS;i++) BuilderSlots[i]=MAX_PLAYERS+1;
...
{
	dcmd(races,5,cmdtext);      // Lists all races in yrace.rr file
	dcmd(gotorace,8,cmdtext);   // Teleports the racer to the start checkpoint
...
dcmd_races(playerid, params[])
{
	...
	new File:f, templine[32], rotfile[]=YRACE_FILE, rraces=-1, rracenames[32][32], idx, string[100];
	...
	while(fread(f,templine,sizeof(templine),false))
	{
		...
		format(string,sizeof(string),"Race %d: '%s'",rraces,rracenames[rraces] );
...
dcmd_editrace(playerid,params[])
{
	...
	format(race_name,sizeof(race_name), RACE_FILE,tmp);
	if(!fexist(race_name))
	{
...
dcmd_saverace(playerid, params[])
{
	if(RaceBuilders[playerid] != 0)
	{
		...
		new race_name[32],templine[42];
		format(race_name, 32, RACE_FILE,tmp);
		if(fexist(race_name))
		{
...
dcmd_deleterace(playerid, params[])
{
	...
	format(filename,sizeof(filename),RACE_FILE,filename);
	if(!fexist(filename))
	{
...
dcmd_join(playerid,params[])
{
	...
	else if(RaceActive==1 && RaceStart==0)
	{
		...
		SendClientMessage(playerid, COLOR_GREEN, "If you dont want to drive there, type /gotorace!");
		Participants++;
	}
...
dcmd_gotorace(playerid,params[])
{
	...
	if(RaceParticipant[playerid] > 0)
	{
		...
		{
			return SendClientMessage(playerid,COLOR_RED,"Unable to teleport at this time: Countdown in progress.");
		}
		if(RaceActive==1 && RaceStart==1)
		{
		    return SendClientMessage(playerid, COLOR_YELLOW, "The race has already started, can't teleport to the start.");
		}
		new Float:x, Float:y, Float:z;
		x = RaceCheckpoints[0][0];
		y = RaceCheckpoints[0][1];
		z = RaceCheckpoints[0][2];
		if(GetPlayerState(playerid) == 2)	SetVehiclePos(GetPlayerVehicleID(playerid),x+2,y+1,z);
		else SetPlayerPos(playerid,x+2,y+1,z);
		return SendClientMessage(playerid, COLOR_GREEN, "You have been teleported to the start checkpoint, line up your car and then type /ready");
	}
	else return SendClientMessage(playerid, COLOR_YELLOW, "You aren't in a race, join first");
}
...
public OnPlayerEnterRaceCheckpoint(playerid)
{
	if(RaceParticipant[playerid]>=1) // See if the player is participating in a race, allows race builders to do their stuff in peace.
	{
		if(RaceParticipant[playerid] == 6) // Player reaches the checkered flag.
	    {
			...
			format(ystring,sizeof(ystring),"RACE:  %s has finished the race, position: %d",name,Ranking);
			...
			if(Racemode == ORacemode && ORacelaps == Racelaps)
			{
				...
				if(racerank == 1)
				{
				    ...
				    new string[128]; format(string,sizeof(string),"RACE: %s has set a TRACK RECORD with a time of %s", name, RaceString);
			...
			if(Ranking<4)
			{
				...
				GivePlayerMoney(playerid,winrar+JoinFee);
				format(ystring,sizeof(ystring),"You have earned $%d from the race!",winrar+JoinFee);
				PrizeMP--;
				SendClientMessage(playerid,COLOR_GREEN,ystring);
			}
			Ranking++;
...
public ChangeLap(playerid)
{
	...
	if(Racemode == ORacemode && ORacelaps == Racelaps)
	{
		...
		format(string,sizeof(string),"RACE:  %s has set a LAP RECORD. Lap %d/%d - time: %s", name, CurrentLap[playerid], Racelaps, TimeString );
		SendClientMessageToAll(COLOR_GREEN,string);
	}
	CurrentLap[playerid]++;
...
{
	new temprace[67], idx;
	format(temprace,sizeof(temprace),RACE_FILE,tmp);
    if(strlen(tmp))
    {
		if(!fexist(temprace))
		{
...
public RaceRotation()
{
	if(!fexist(YRACE_FILE))
	{
	    printf("ERROR in  YRACE's Race Rotation (TurfWar/Races/yrace.rr): yrace.rr doesn't exist!");
	    return -1;
	}
	...
	new File:f, templine[32], rotfile[]=YRACE_FILE, rraces=-1, rracenames[32][32], idx, fback;
	f = fopen(rotfile, io_read);
	while(fread(f,templine,sizeof(templine),false))
	{
		idx = 0;
